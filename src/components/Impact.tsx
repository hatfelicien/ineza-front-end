...
import { FaUserGraduate, FaHandshake, FaLaptopHouse } from 'react-icons/fa';
import { FaPeopleRoof } from 'react-icons/fa6';
import { MdBarChart } from 'react-icons/md';

const stats = [
  { icon: <FaUserGraduate className="w-5 h-5 sm:w-7 sm:h-7 text-white" />, label: 'Children Educated', suffix: '+', color: 'bg-accent' },
  { icon: <FaLaptopHouse className="w-5 h-5 sm:w-7 sm:h-7 text-white" />, label: 'Communities Served', suffix: '+', color: 'bg-orange' },
  { icon: <FaPeopleRoof className="w-5 h-5 sm:w-7 sm:h-7" style={{ color: '#8B4513' }} />, label: 'Families Supported', suffix: '+', color: 'bg-gold' },
  { icon: <FaHandshake className="w-5 h-5 sm:w-7 sm:h-7 text-white" />, label: 'Volunteers', suffix: '+', color: 'bg-secondary' },
];
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div ref={ref} className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-6 mb-10 sm:mb-16">
            {stats.map((stat, i) => (
              ...
              >
                <div className={`w-9 h-9 sm:w-14 sm:h-14 ${stat.color} rounded-xl flex items-center justify-center mx-auto mb-2 sm:mb-4 shadow-lg`}>
                  {stat.icon}
                </div>
                  ...
                  <div key={item.label}>
                    ...
                    <div className="h-2.5 bg-white/10 rounded-full overflow-hidden">
                      <div
                        ...
                        style={{ width: isVisible ? `${item.value}%` : '0%', transitionDelay: `${400 + i * 150}ms` }}
                      />
                    </div>
                  </div>
                ))}
