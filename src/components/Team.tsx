...
const Team: React.FC = () => {
  ...
  return (
    <section id="team" className="py-14 sm:py-24 bg-white dark:bg-gray-900">
      <div className="w-full px-4 sm:px-6 max-w-6xl mx-auto">
        <div className={`text-center mb-10 transition-all duration-1000 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
          <h2 className="text-2xl sm:text-4xl font-bold text-gray-800 dark:text-white mb-3">Our Team</h2>
        ...
        <div ref={ref} className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-8">
          {teamMembers.map((member, index) => (
            <div
              key={index}
              className={`flip-card h-72 sm:h-96 transition-all duration-700 cursor-pointer ${
